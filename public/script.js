  ...
  userManagement: {
    ...
    fields: [
      ...
      { key: "password", label: "Password", placeholder: "Password", type: "password", required: true, table: false },
      {
        ...
        label: "Role",
        type: "select",
        options: ["User", "Admin", "Administrator"],
        required: true
      },
...
async function pollCloudData() {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Cloud poll failed with status ${response.status}`);
...
function defaultedRecordFieldValue(record, field) {
  ...
  if (record || (state.activeView !== "modification" && state.activeView !== "testCases")) return recordValue;
...
function applyFirstProjectDefaultsToForm() {
  if (state.activeView !== "modification" && state.activeView !== "testCases") return {};
  ...
  Object.entries(defaults).forEach(([key, value]) => {
    ...
    field.value = key.toLowerCase().includes("date") ? parseDateKey(value) : value;
  });
...
function currentUserRole() {
  return String(state.currentUser?.role || "User").trim() || "User";
}
...
function tableFieldGroups(viewKey, view) {
  ...
  return {
    beforeGenerated: fields.filter(field => field.key !== "status"),
    afterGenerated: fields.filter(field => field.key === "status")
  };
}
  ...
  if (!detailRecords.length) {
    ...
    sortedDetailRecords.forEach(record => {
      ...
      columns.forEach(column => {
        ...
        if (column.key === "details" || column.key === "remarks") {
...
function showForm(record = null) {
  ...
  if (!record && state.activeView === "userManagement" && !isCurrentAdmin()) {
    window.alert("Only the admin can add User Management records.");
    return;
  }
  ...
  view.fields.forEach(field => {
    ...
    if (field.type !== "file") input.value = field.type === "date" ? parseDateKey(recordValue) : recordValue;
...
function switchView(viewKey) {
  if (viewKey === "userManagement" && !canViewUserManagement()) {
    window.alert("Only the admin can view User Management.");
    viewKey = "dashboard";
  }
...
async function handleSave(event) {
  ...
  if (state.activeView === "testCases" && baselineDefaults.qa) {
    payload.qa = baselineDefaults.qa;
  }
...
function render(options = {}) {
  ...
  records.forEach(record => {
    ...
    cells.forEach(({ field, value }) => {
      const cell = document.createElement("td");
      cell.textContent = value || "-";
      if (field?.key === "details" || field?.key === "remarks") {
...
if (normalizedIds || normalizedTcIds || normalizedStatuses || normalizedTestCaseStatuses) {
  saveLocalData();
}
render();
initializeLogin();
